...
def run_mcp(calls, wait=2):
    ...
    send({"jsonrpc":"2.0","id":1,"method":"initialize","params":{
        ...
        "clientInfo":{"name":"check-exists","version":"1"}}})
    time.sleep(0.3)
...
local function dump(inst, depth)
	...
	local result = pad .. inst.Name .. " (" .. inst.ClassName .. ")\n"
	for _, child in ipairs(inst:GetChildren()) do
		result = result .. dump(child, depth + 1)
	end
	return result
end
return dump(game:GetService("ServerScriptService"))
'''

calls = [{"name": "execute_luau", "arguments": {"code": code}}]
