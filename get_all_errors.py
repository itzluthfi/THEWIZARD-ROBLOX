...
def run_mcp(calls, wait=2):
    ...
    send({"jsonrpc":"2.0","id":1,"method":"initialize","params":{
        ...
        "clientInfo":{"name":"get-errors","version":"1"}}})
    time.sleep(0.3)
...
print("=== SERVER AND CLIENT ERRORS/WARNINGS ===")
for msg in messages:
    txt = msg.get("message", "")
    # Print errors, warnings, stack traces, or script initialization messages
    if "is not a valid member" in txt or "Error" in txt or "fail" in txt or "Stack" in txt or "Line" in txt or "ServerScriptService" in txt or "Bootstrap" in txt or "MobService" in txt or "Gacha" in txt:
        print(f"[{msg.get('timestamp')}] {txt}")
